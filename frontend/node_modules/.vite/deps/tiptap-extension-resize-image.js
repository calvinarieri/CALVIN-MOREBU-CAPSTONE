import { Dn as DOMSerializer, Nt as isNodeEmpty, Sn as PluginKey, Xt as nodeInputRule, bn as NodeSelection, m as ResizableNodeView, qt as mergeAttributes, u as Node3, vn as Decoration, xn as Plugin, yn as DecorationSet } from "./dist-2cwDIgx3.js";
...
//#endregion
//#region node_modules/tiptap-extension-resize-image/esm/index.js
var CONSTANTS = {
...
var utils = {
	...
	removeResizeElements(container) {
		container.querySelectorAll("[data-resize-image-ui]").forEach((element) => element.remove());
	}
};
var StyleManager = class {
	static getContainerStyle(inline, width) {
		return `${`width: ${width || "100%"}; height: auto; cursor: pointer;`} ${inline ? "display: inline-block;" : ""}`;
	}
	static getWrapperStyle(inline) {
		return inline ? "display: inline-block; float: left; padding-right: 8px;" : "display: flex; margin: 0;";
	}
...
* stripped silently.
*/
var DIMENSION_TOKEN = /^-?\d+(?:\.\d+)?(?:px|em|rem|%|vw|vh)$|^auto$|^0$/i;
var isDimensionValue = (value) => {
	return value.trim().split(/\s+/).every((token) => DIMENSION_TOKEN.test(token));
};
var isOneOf = (allowed) => (value) => allowed.includes(value.trim().toLowerCase());
...
var AttributeParser = class {
	static parseImageAttributes(nodeAttrs, imgElement) {
		Object.entries(nodeAttrs).forEach(([key, value]) => {
			if (value === void 0 || value === null || key === "wrapperStyle") return;
			if (key === "containerStyle") {
				const width = value.match(/width:\s*([0-9.]+)px/);
				if (width) imgElement.setAttribute("width", width[1]);
				return;
			}
...
function clampWidth(width, limits) {
	...
	let clampedWidth = Math.max(minWidth !== void 0 ? Math.max(0, minWidth) : 0, width);
	if (maxWidth !== void 0 && clampedWidth > maxWidth) clampedWidth = maxWidth;
	return clampedWidth;
}
/**
...
var detachIfEmpty = () => {
	...
	attachedDoc === null || attachedDoc === void 0 || attachedDoc.removeEventListener("click", dispatch);
	attached = false;
	attachedDoc = null;
};
/**
...
var PositionController = class {
	...
	handleLeftClick() {
		if (!this.inline) this.elements.container.style.margin = "0 auto 0 0";
		else this.applyInlineFloat("left");
	...
	handleRightClick() {
		if (!this.inline) this.elements.container.style.margin = "0 0 0 auto";
		else this.applyInlineFloat("right");
		this.dispatchNodeView();
	}
...
var ResizeController = class {
	constructor(elements, dispatchNodeView, resizeLimits = {}) {
		...
		this.handleMouseUp = () => {
			if (this.state.isResizing) this.state.isResizing = false;
...
var ImageNodeView = class {
	constructor(context, inline, resizeLimits = {}) {
		...
		this.handleContainerClick = () => {
			...
			isMobile && (editorDom === null || editorDom === void 0 || editorDom.blur());
		...
		this.handleDocumentClick = (e) => {
			...
			if (!(!!target && (this.elements.container.contains(target) || !!target.closest("[data-resize-image-ui]")))) {
		...
		this.dispatchNodeView = () => {
			...
			if (typeof getPos === "function") {
				...
				const newAttrs = Object.assign(Object.assign({}, this.context.node.attrs), {
					width: (_a = AttributeParser.extractWidthFromStyle(containerStyle)) !== null && _a !== void 0 ? _a : this.context.node.attrs.width,
					containerStyle,
					wrapperStyle
				});
		...
		this.destroy = () => {
			...
			(_a = this.unsubscribeDocumentClick) === null || _a === void 0 || _a.call(this);
			this.unsubscribeDocumentClick = null;
			this.removeResizeElements();
		};
		/**
		...
		*/
		this.update = (node) => {
			...
			if (prev.attrs.wrapperStyle !== node.attrs.wrapperStyle) this.elements.wrapper.setAttribute("style", sanitizeStyle(node.attrs.wrapperStyle));
			if (prev.attrs.containerStyle !== node.attrs.containerStyle) this.elements.container.setAttribute("style", sanitizeStyle(node.attrs.containerStyle));
			if (prev.attrs.src !== node.attrs.src || prev.attrs.alt !== node.attrs.alt || prev.attrs.title !== node.attrs.title || prev.attrs.containerStyle !== node.attrs.containerStyle) this.setupImageAttributes();
			this.applyResizeLimits();
			return true;
		};
	...
	*/
	...
	createPositionController() {
		new PositionController(this.elements, this.inline, this.dispatchNodeView).createPositionControls();
	}
	createResizeHandler() {
		...
		return Object.assign(Object.assign({}, (_a = this.parent) === null || _a === void 0 ? void 0 : _a.call(this)), {
			containerStyle: {
				...
				parseHTML: (element) => {
					...
					return width ? StyleManager.getContainerStyle(inline, `${width}px`) : sanitizeStyle(element.style.cssText);
				}
			},
			wrapperStyle: {
...
var ImageResize = index_default.extend(imageResizeConfig);
var FigureNodeView = class extends ImageNodeView {
	constructor() {
		super(...arguments);
		this.dispatchNodeView = () => {
			var _a;
			...
			if (typeof getPos === "function") {
				...
				const newSelection = selection instanceof NodeSelection ? NodeSelection.create(tr.doc, selection.from) : selection.map(tr.doc, tr.mapping);
...
function findEnclosingFigure(state) {
	...
	if (selection instanceof NodeSelection && selection.node.type.name === "figure") return {
		node: selection.node,
		pos: selection.from
	};
...
var Figure = ImageResize.extend({
	...
	addAttributes() {
		return {
			src: {
				...
				parseHTML: (element) => {
					var _a;
					return (_a = element.querySelector("img")) === null || _a === void 0 ? void 0 : _a.getAttribute("src");
				}
			},
			alt: {
				default: null,
				parseHTML: (element) => {
					var _a;
					return (_a = element.querySelector("img")) === null || _a === void 0 ? void 0 : _a.getAttribute("alt");
				}
			},
			title: {
				default: null,
				parseHTML: (element) => {
					var _a;
					return (_a = element.querySelector("img")) === null || _a === void 0 ? void 0 : _a.getAttribute("title");
				}
			},
			containerStyle: {
				default: null,
	...
	addNodeView() {
		return ({ node, editor, getPos }) => {
			...
			return new FigureNodeView({
				node,
				editor,
				view: editor.view,
				getPos: typeof getPos === "function" ? getPos : void 0
			}, false, resizeLimits).initializeFigure();
		};
	},
	addCommands() {
		return {
			...
			*/
			addCaption: (caption = "") => ({ chain, state }) => {
				...
				}, figureNode.toJSON(), { updateSelection: false }).command(({ commands }) => {
			...
			*/
			toggleCaption: (caption = "") => ({ state, commands }) => {
				...
				const node = state.doc.nodeAt(from);
				if ((node === null || node === void 0 ? void 0 : node.type.name) === "imageResize") return commands.addCaption(caption);
